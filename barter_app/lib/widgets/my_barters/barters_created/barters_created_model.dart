...
class TCreatedModel extends ChangeNotifier {
  final api = ApiClient();
  Map<GGetCreatedData_GetCreated, Map<String, GGetProductData_Product>> data =
      {};
  ...
  Future<void> getList() async {
    try {
      ...
      var transactionList = await api.getTransactionCreated();
  ...
  Future<bool> AcceptTransaction(String id) async {
    var ok = await api.updateTransactionOngoing(id: id);
    return ok;
  }

  Future<bool> DeclinedTransaction(String id) async {
    var ok = await api.updateTransactionDeclined(id: id);
    return ok;
  }
}
